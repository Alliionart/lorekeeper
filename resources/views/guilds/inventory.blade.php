                        ...
                        <div class="row mb-3">
                            @foreach ($chunk as $itemId => $stack)
                                ...
                                <div class="col-sm-3 col-6 text-center inventory-item" data-id="{{ $stack->first()->pivot->id }}"
                                    data-name="{!! $canName && $stackName ? htmlentities($stackNameClean) . ' [' : null !!}{{ $guild->name ? $guild->name : $guild->id }}'s {{ $stack->first()->name }}{!! $canName && $stackName ? ']' : null !!}">
                                    <div class="mb-1">
                        ...
                        <div class="col-lg-3 col-sm-4 col-12">
                            ...
                            <ul class="mb-0">
                                @foreach ($itemtype as $item)
                                    ...
                                    $stackNameClean = htmlentities($stackName);
                                    ?>
                                    <div data-id="{{ $item->pivot->id }}" data-name="{!! $canName && $stackName ? htmlentities($stackNameClean) . ' [' : null !!}{{ $guild->name ? $guild->name : $guild->id }}'s {{ $item->name }}{!! $canName && $stackName ? ']' : null !!}">
                                        <li>
    ...
    <div class="mb-4 logs-table">
        <div class="logs-table-header">
            <div class="row">
                ...
                <div class="col-6 col-md-2">
                    <div class="logs-table-cell">Guild</div>
                </div>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <p>Note that granting items does not check against any category hold limits for guilds.</p>
                        <div class="form-group">
                            {!! Form::open(['url' => 'admin/guilds/' . $guild->id . '/grant-items']) !!}
                            ...
                            <div class="form-group">
                                ...
                                {!! Form::label('disallow_transfer', 'Guild-bound', ['class' => 'form-check-label ml-3']) !!} {!! add_help('If this is on, the guild\'s owner will not be able to transfer this item to their inventory. Items that disallow transfers by default will still not be transferrable.') !!}
                            </div>
    ...
    <script>
        $(document).ready(function() {
            $('.inventory-stack').on('click', function(e) {
                ...
                loadModal("{{ url('items') }}/guild/" + $parent.data('id'), $parent.data('name'));
            });
