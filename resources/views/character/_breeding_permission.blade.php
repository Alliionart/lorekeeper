                    ...
                    @if (Auth::check() && (Auth::user()->id == $permission->recipient_id || Auth::user()->hasPower('manage_characters')) && !$permission->is_used)
                        <div class="float-right">
                            @if (Auth::user()->id == $permission->recipient_id || Auth::user()->hasPower('manage_characters'))
                                <a href="#" class="btn btn-sm btn-primary transfer-breeding-permission" data-id="{{ $permission->id }}"
                                    data-slug="{{ $character->slug }}">{{ Auth::user()->id != $permission->recipient_id && Auth::user()->hasPower('manage_characters') ? '(Admin)' : '' }} Transfer</a>
                            @endif
