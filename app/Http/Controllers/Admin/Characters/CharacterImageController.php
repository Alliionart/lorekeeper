...
class CharacterImageController extends Controller {
    ...
    */
    ...
    public function postNewImage(Request $request, CharacterManager $service, $slug) {
        ...
        $data = $request->only(['image', 'thumbnail', 'x0', 'x1', 'y0', 'y1', 'use_cropper', 'artist_url', 'artist_id', 'artist_type', 'designer_url', 'designer_id', 'designer_type', 'species_id', 'subtype_id', 'rarity_id', 'feature_id', 'feature_data', 'is_valid', 'is_visible', 'transformation_id', 'transformation_info', 'transformation_description']);
        $this->character = Character::where('slug', $slug)->first();
        if (!$this->character) {
            abort(404);
        }
    ...
    public function postEditImageCredits(Request $request, CharacterManager $service, $id) {
        $data = $request->only(['artist_url', 'artist_id', 'artist_type', 'designer_url', 'designer_id', 'designer_type']);
        $image = CharacterImage::find($id);
        if (!$image) {
            abort(404);
        }
