...
class ResearchService extends Service {
    ...
    */
     ...
     * @param User                  $user
     *
     * @return \App\Models\Tree\Tree|bool
     */
    public function updateTree($tree, $data, $user) {
        ...
        try {
            ...
            if (Tree::where('name', $data['name'])->where('id', '!=', $tree->id)->exists()) {
    ...
    */
    ...
    public function updateResearch($research, $data, $user) {
        ...
        try {
            // More specific validation
            if (Research::where('name', $data['name'])->where('id', '!=', $research->id)->exists()) {
                throw new \Exception('The name has already been taken.');
            }
            ...
            if (!$this->updateChildrenOnTreeChange($research, $data['tree_id'])) {
    ...
    public function grantResearch($data, $staff) {
        ...
        try {
            ...
            if (!count($researches)) {
                throw new \Exception('No valid researches found.');
            }
            ...
            foreach ($users as $user) {
                foreach ($researches as $research) {
                    ...
                    if ($userResearch) {
                        throw new \Exception($user->displayName.' already has '.$research->displayName.'.');
                        ...
                        $researchLog = ResearchLog::create([
                            ...
                            'data'         => json_encode(['log_type' => 'Staff Grant', 'message' => $data['message'] ?? null]),
    ...
    private function populateResearchData($data, $research = null) {
        ...
        $saveData['icon_code'] = $data['icon'] ?? 'fas fa-sitemap';

        $saveData['tree_id'] = $data['tree_id'];

        $saveData['parent_id'] = isset($data['parent_id']) && $data['parent_id'] ? $data['parent_id'] : null;
        $saveData['price'] = $data['price'] ?? 0;

        if (isset($data['prereq_is_same']) && $data['prereq_is_same']) {
            $saveData['prerequisite_id'] = $saveData['parent_id'];
        } else {
            $saveData['prerequisite_id'] = isset($data['prerequisite_id']) && $data['prerequisite_id'] ? $data['prerequisite_id'] : null;
        }

        $saveData['is_active'] = isset($data['is_active']);

        $saveData['rewardable_type'] = $data['rewardable_type'] ?? null;
        $saveData['rewardable_id'] = $data['rewardable_id'] ?? null;
        $saveData['quantity'] = $data['quantity'] ?? null;
