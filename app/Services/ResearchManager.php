    ...
    */
    ...
    public function buyResearch($research_id, $user) {
        ...
        try {
            ...
            if ($userCurrency->quantity < $research->price) {
                throw new \Exception("You don't have enough of the currency.");
            }

            $userResearch = UserResearch::where('user_id', $user->id)->where('research_id', $research->id)->first();
            if ($userResearch) {
                throw new \Exception('You already have this research.');
            }

            if ($research->price > 0 && !(new CurrencyManager)->debitCurrency($user, null, 'Research Purchase', 'Purchased '.$research->name.' from '.$research->tree->name, $research->tree->currency, $research->price)) {
            ...
            if (!$this->creditResearch(null, $user, 'Research Purchase', $researchLog, $research)) {
                throw new \Exception('Failed to purchase research.');
            }
     ...
     * @return bool
     */
    public function creditResearch($sender, $recipient, $type, $data, $research) {
        DB::beginTransaction();
    ...
    public function getRewards($research, $user) {
        ...
        try {
            ...
            if (!$rewards = fillUserAssets($researchRewards, null, $userResearch->user, $researchLogType, $researchData)) {
