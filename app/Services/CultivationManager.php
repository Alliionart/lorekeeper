    ...
    */
    ...
    public function preparePlot($userToolId, $plotNumber, $areaId) {
        ...
        try {
            ...
            if (!isset($userTool)) {
                throw new \Exception('Tool could not be found.');
            }
            $userArea = UserArea::find($areaId);
            if (!isset($userArea)) {
                throw new \Exception('Area could not be found.');
            }

            $tag = $userTool->item->tag('tool');
            if (!isset($tag) || !isset($tag->data['plot_id'])) {
                throw new \Exception('Tool is missing its item tag information.');
            }

            if (!$userArea->area->allowedPlots->pluck('id')->contains((int) $tag->data['plot_id'])) {
                throw new \Exception('This tool cannot be used for this area.');
            }
    ...
    public function cultivatePlot($userSeedId, $plotNumber, $areaId) {
        ...
        try {
            ...
            if (!isset($tag)) {
                throw new \Exception('Seed is missing its item tag.');
            }

            $userPlot = UserPlot::where('user_id', $userSeed->user_id)->where('plot_number', $plotNumber)->where('user_area_id', $areaId)->first();

            if (isset($userPlot)) {
                if (!$userPlot->plot->allowedItems->pluck('id')->contains($userSeed->item->id)) {
                    throw new \Exception('This item cannot be cultivated on this plot.');
                }
    ...
    public function tendPlot($plotId) {
        ...
        try {
            ...
            if (!isset($userPlot)) {
                throw new \Exception('Plot could not be found.');
            }
            ...
            if ($this->canTend($userPlot->tended_at, $date)) {
                ...
                if (Settings::get('cultivation_care_cooldown') > 0 && $caredPlots->count() >= Settings::get('cultivation_care_cooldown')) {
                    throw new \Exception('You already tended to '.$caredPlots->count().' plot(s) today.');
                }
    ...
    public function harvestPlot($plotId, $user) {
        ...
        try {
            ...
            if ($userPlot->stage == 5) {
                ...
                if (!isset($seedTag)) {
                    throw new \Exception('Seed tag data could not be found.');
                }
