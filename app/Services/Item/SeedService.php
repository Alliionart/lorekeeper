    ...
    */
    ...
    public function updateData($tag, $data) {
        ...
        if (!isset($data['rewardable_type'])) {
            throw new \Exception('Please set at least one reward.');
        }
        if (isset($data['stage_2_days']) && $data['stage_2_days'] <= 0) {
            throw new \Exception('Stage 2 days must be greater than 0.');
        }
        if (isset($data['stage_3_days']) && $data['stage_3_days'] <= 0) {
            throw new \Exception('Stage 3 days must be greater than 0.');
        }
        if (isset($data['stage_4_days']) && $data['stage_4_days'] <= 0) {
            throw new \Exception('Stage 4 days must be greater than 0.');
        }
        ...
        try {
            ...
            if (isset($data['stage_2_days'])) {
                $assets['stage_2_days'] = $data['stage_2_days'];
            }
            if (isset($data['stage_3_days'])) {
                $assets['stage_3_days'] = $data['stage_3_days'];
            }
            if (isset($data['stage_4_days'])) {
                $assets['stage_4_days'] = $data['stage_4_days'];
            }
            ...
            if ($image) {
                $this->handleImage($image, public_path('images/data/items/seeds/'), $saveName);
            } else {
                $assets['stage_5_image'] = ($tag->getData() ? $tag->getData()['stage_5_image'] : null);
            }
