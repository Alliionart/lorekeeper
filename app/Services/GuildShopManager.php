...
class GuildShopManager extends Service {
    ...
    |--------------------------------------------------------------------------
    |
    | Handles purchasing of items from guild shops.
    |
    */

    /**
     * Buys an item from a guild shop.
     *
     * @param array                 $data
    ...
    public function buyStock($data, $user) {
        ...
        try {
            ...
            $shopStock = GuildShopStock::where('id', $data['stock_id'])->where('guild_shop_id', $data['guild_shop_id'])->with('currency')->with('item')->first();
            if (!$shopStock) {
    ...
    public function checkUserPurchases($shopStock, $user) {
        return GuildShopLog::where('guild_shop_id', $shopStock->guild_shop_id)->where('item_id', $shopStock->item_id)->where('user_id', $user->id)->sum('quantity');
    }
